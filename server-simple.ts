...
// Define a simple user type for testing authentication
interface UserType {
  id: number;
  username: string;
  displayName: string;
...
function generateStreamKey(userId: number): string {
 ...
 */
function hasValidKeyFormat(streamKey: string | null | undefined): boolean {
...
function validateStreamKey(streamKey: string, userId: number): boolean {
...
app.post('/api/streams/validate-key', (req: Request, res: Response) => {
  // Ensure user is authenticated
  if (!req.user) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
