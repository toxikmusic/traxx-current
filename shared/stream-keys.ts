 ...
 * @returns The generated stream key
 */
 ...
 * 
 * @param streamKey - The stream key to validate
 * @param userId - The user ID to validate against
 * @param expiryHours - Optional custom expiry time in hours
 * @returns Whether the key is valid for this user
 */
export function validateStreamKey(streamKey: string, userId: number, expiryHours = 24): boolean {
 ...
 */
export function extractUserIdFromStreamKey(streamKey: string): number | null {
  const parts = streamKey.split(':');
  if (parts.length !== 3) {
    return null;
  }
 ...
 * 
 * @param streamKey - The private stream key
 * @returns A public stream ID
 */
export function generatePublicStreamId(streamKey: string): string {
 ...
 * 
 * @param streamKey - The private stream key to validate
 * @param publicId - The public stream ID to validate against
 * @returns Whether they match
 */
export function validateStreamKeyWithPublicId(streamKey: string, publicId: string): boolean {
 ...
 * @returns Whether the key has a valid format (userId:timestamp:signature)
 */
export function hasValidKeyFormat(streamKey: string | null | undefined): boolean {
